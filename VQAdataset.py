...
class VQADataset(Dataset):
    def __init__(self, args, datasets, status='train'):
        ...
        self.fusion_features, self.length, self.label, self.KCL, self.N = dict(), dict(), dict(), dict(), dict()
        for dataset in datasets:
            N = len(self.index[dataset])
            self.N[dataset] = N
            self.fusion_features[dataset] = np.zeros((N, max_len_all, args.feat_dim), dtype=np.float32)
            self.length[dataset] = np.zeros(N, dtype=np.int)
            self.label[dataset] = np.zeros((N, 1), dtype=np.float32)
            self.KCL[dataset] = []
            for i in range(N):
                fusion_features = np.load(args.features_dir[dataset] + str(self.index[dataset][i]) + '_' + 'fusion_features.npy')  #load fusion_features
                self.fusion_features[dataset][i, :fusion_features.shape[0], :] = fusion_features
                ...
                self.length[dataset][i] = fusion_features.shape[0]

    def __len__(self):
        return max(self.N.values())

    def __getitem__(self, idx):
        data = [(self.fusion_features[dataset][idx % self.N[dataset]],
